...
class DataTransformer:
    ...
    def transform(self, ch, method, properties, body):
        ...
        data = data[["countriesAndTerritories", "cases_weekly", "deaths_weekly"]]
        data = data.groupby(by="countriesAndTerritories").sum()
        data = data.merge(population, on="countriesAndTerritories")
        data["cases_rate"] = data["cases_weekly"] * 100000 / data["popData2019"]
        data["deaths_rate"] = data["deaths_weekly"] * 100000 / data["popData2019"]
        assert isinstance(data, pd.DataFrame)
        data.sort_values(by="cases_weekly", inplace=True, ascending=False)
